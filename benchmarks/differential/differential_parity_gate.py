...
def run_harness(output_dir: Path | None = None, run_count: int = 20) -> Dict[str, Any]:
    ...
    suspected_global_dependency_count = 0
    candidate_size_values: List[int] = []
    fresh_size_values: List[int] = []
    size_delta_values: List[int] = []
    container_metadata_equal_count = 0
    payload_order_equal_count = 0
    zstd_frame_equal_count = 0
    msgpack_structure_equal_count = 0
    ...
    for scenario, updater in WORKLOADS.items():
        ...
        with tempfile.TemporaryDirectory(prefix=f"mc_parity_{scenario}_") as tmp:
            ...
            scenario_parity = 0
            scenario_total = 0
            scenario_mismatch_stage_counts: Dict[str, int] = {}
            scenario_first_offsets: List[int] = []
            ...
            for i in range(run_count):
                ...
                mismatch_stage = str(result.report.get("mismatch_stage", "none"))
                mismatch_stage_counts[mismatch_stage] = (
                    mismatch_stage_counts.get(mismatch_stage, 0) + 1
                )
                scenario_mismatch_stage_counts[mismatch_stage] = (
                    scenario_mismatch_stage_counts.get(mismatch_stage, 0) + 1
                )
                first_offset = int(result.report.get("mismatch_first_byte_offset", -1))
                if first_offset >= 0:
                    scenario_first_offsets.append(first_offset)
                if bool(result.report.get("suspected_global_dependency", False)):
                    suspected_global_dependency_count += 1
                candidate_size_values.append(
                    int(result.report.get("candidate_size", 0))
                )
                fresh_size_values.append(int(result.report.get("fresh_size", 0)))
                size_delta_values.append(int(result.report.get("size_delta", 0)))
                if bool(result.report.get("container_metadata_equal", False)):
                    container_metadata_equal_count += 1
                if bool(result.report.get("payload_order_equal", False)):
                    payload_order_equal_count += 1
                if bool(result.report.get("zstd_frame_equal", False)):
                    zstd_frame_equal_count += 1
                if bool(result.report.get("msgpack_structure_equal", False)):
                    msgpack_structure_equal_count += 1
            ...
            workload_rows.append(
                {
                    ...
                    "runtime_mismatch_stage_counts": dict(
                        sorted(scenario_mismatch_stage_counts.items())
                    ),
                    "runtime_mismatch_first_offset_min": (
                        min(scenario_first_offsets) if scenario_first_offsets else -1
                    ),
    ...
    payload = {
        ...
        "suspected_global_dependency_rate": (
            (suspected_global_dependency_count / total) if total else 0.0
        ),
        "candidate_size_avg": (
            (sum(candidate_size_values) / len(candidate_size_values))
            if candidate_size_values
            else 0.0
        ),
        "fresh_size_avg": (
            (sum(fresh_size_values) / len(fresh_size_values))
            if fresh_size_values
            else 0.0
        ),
        "size_delta_avg": (
            (sum(size_delta_values) / len(size_delta_values))
            if size_delta_values
            else 0.0
        ),
        "container_metadata_equal_rate": (
            (container_metadata_equal_count / total) if total else 0.0
        ),
        "payload_order_equal_rate": (
            (payload_order_equal_count / total) if total else 0.0
        ),
        "zstd_frame_equal_rate": (zstd_frame_equal_count / total) if total else 0.0,
        "msgpack_structure_equal_rate": (
            (msgpack_structure_equal_count / total) if total else 0.0
        ),
