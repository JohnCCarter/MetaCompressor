...
def _run_workload(
    ...
    with tempfile.TemporaryDirectory(prefix="mc_partial_reuse_sim_") as tmp:
        ...
        for i in range(run_count):
            ...
            mismatch_stage = str(report.get("mismatch_stage", "none"))
            mismatch_stage_counts[mismatch_stage] = (
                mismatch_stage_counts.get(mismatch_stage, 0) + 1
            )
            offset = int(report.get("mismatch_first_byte_offset", -1))
            if offset >= 0:
                mismatch_offsets.append(offset)
            if bool(report.get("suspected_global_dependency", False)):
                suspected_global_dependency_count += 1
            candidate_sizes.append(int(report.get("candidate_size", 0)))
            fresh_sizes.append(int(report.get("fresh_size", 0)))
            size_deltas.append(int(report.get("size_delta", 0)))
            if bool(report.get("container_metadata_equal", False)):
                container_metadata_equal_count += 1
            if bool(report.get("payload_order_equal", False)):
                payload_order_equal_count += 1
            if bool(report.get("zstd_frame_equal", False)):
                zstd_frame_equal_count += 1
            if bool(report.get("msgpack_structure_equal", False)):
        ...
        return {
            ...
            "runtime_mismatch_stage_counts": dict(
                sorted(mismatch_stage_counts.items())
            ),
            "runtime_mismatch_first_offset_min": (
                min(mismatch_offsets) if mismatch_offsets else -1
            ),
            "runtime_mismatch_first_offset_avg": (
                mean(mismatch_offsets) if mismatch_offsets else -1.0
            ),
            "suspected_global_dependency_rate": (
                float(suspected_global_dependency_count) / float(run_count)
                if run_count > 0
                else 0.0
            ),
            "candidate_size_avg": mean(candidate_sizes) if candidate_sizes else 0.0,
            "fresh_size_avg": mean(fresh_sizes) if fresh_sizes else 0.0,
            "size_delta_avg": mean(size_deltas) if size_deltas else 0.0,
            "container_metadata_equal_rate": (
                float(container_metadata_equal_count) / float(run_count)
                if run_count > 0
                else 0.0
            ),
            "payload_order_equal_rate": (
                float(payload_order_equal_count) / float(run_count)
                if run_count > 0
                else 0.0
            ),
            "zstd_frame_equal_rate": (
                float(zstd_frame_equal_count) / float(run_count)
                if run_count > 0
                else 0.0
            ),
            "msgpack_structure_equal_rate": (
                float(msgpack_structure_equal_count) / float(run_count)
                if run_count > 0
                else 0.0
            ),
