...
from benchmarks.acceptance import production_validation as pv

_RESULTS_DIR = REPO_ROOT / "results" / "hardening"
...
_DEBUG_LOG_PATH = REPO_ROOT / "tmp" / "logs" / "debug-7c93f6.log"
...
def _analysis_skip_eligibility(
    *,
    receipt: Optional[Dict[str, Any]],
    differential_report: Optional[Dict[str, Any]],
) -> tuple[bool, str]:
    """Fail-closed analysis-skip eligibility for quick warm-path."""
    if receipt is None:
        return False, "stale_or_missing_receipt"
    if differential_report is None:
        return False, "missing_differential_report"
    if not bool(differential_report.get("previous_manifest_valid", False)):
        return False, "invalid_or_missing_manifest"
    if not bool(differential_report.get("reuse_allowed", False)):
        return False, "manifest_reuse_not_allowed"
    if int(differential_report.get("changed_chunk_count", 0)) > 0:
        return False, "changed_chunk_detected"
    if int(differential_report.get("new_chunk_count", 0)) > 0:
        return False, "new_chunk_detected"
    if int(differential_report.get("deleted_chunk_count", 0)) > 0:
        return False, "deleted_chunk_detected"
    if int(differential_report.get("rescan_chunk_count", 0)) > 0:
        return False, "rescan_chunks_present"
    if not str(receipt.get("selected_path_hint", "")).strip():
        return False, "receipt_metadata_mismatch"
    if (
        _safe_float(receipt.get("confidence_score", 0.0))
...
def _measure_dataset_quick(
    ...
    if differential_report_enabled:
        ...
        manifest_validation_time_ms = int((time.perf_counter() - manifest_t0) * 1000.0)
    ...
    if not differential_report_enabled:
        ...
        # Warm-path skip requires fully valid receipt + manifest state.
        analysis_skip_eligible, analysis_skip_denied_reason = (
            _analysis_skip_eligibility(
                receipt=receipt,
                differential_report=differential_report,
            )
        )
        if analysis_skip_eligible and receipt is not None:
            ...
            analysis_skip_saved_estimate_ms = int(
                max(0, int(receipt.get("bounded_scan_time_ms_estimate", 0)))
            )
            if rescan_chunk_ids:
                analysis_skip_eligible = False
                analysis_skip_denied_reason = "rescan_chunks_present"
                forced_rescan_reason = analysis_skip_denied_reason
                scan_t0 = time.perf_counter()
                try:
                    features = _decision_kernel_features_for_chunk_ids(
                        dataset_dir=dataset_dir,
                ...
                warm_path_used = True
                warm_path_saved_estimate_ms = int(analysis_skip_saved_estimate_ms)
                features = _receipt_to_features(receipt, base_features)
                receipt_used = True
        else:
            if not analysis_skip_denied_reason and advisory_receipt is not None:
                analysis_skip_denied_reason = "receipt_metadata_mismatch"
            elif not analysis_skip_denied_reason:
                analysis_skip_denied_reason = "stale_or_missing_receipt"
            forced_rescan_reason = analysis_skip_denied_reason
            scan_t0 = time.perf_counter()
            try:
                features = _decision_kernel_features(dataset_dir)
    ...
    decision_report = {
        ...
        "manifest_validation_time_ms": int(manifest_validation_time_ms),
        "decision_kernel_total_time_ms": decision_kernel_total_time_ms,
        "receipt_reuse_saved_scan_estimate_ms": saved_scan_estimate_ms,
        "analysis_skip_eligible": bool(analysis_skip_eligible),
        "analysis_skip_used": bool(analysis_skip_used),
        "analysis_skip_denied_reason": analysis_skip_denied_reason,
        ...
        "warm_path_used": bool(warm_path_used),
        "warm_path_saved_estimate_ms": int(warm_path_saved_estimate_ms),
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Directory for markdown/json results (default: results/hardening/).",
    )
    parser.add_argument(
        "--mode",
