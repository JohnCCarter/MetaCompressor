...
4. **Layout**: follow **`docs/policy/repository-layout-policy.md`**; Python path rules are enforced by **`scripts/check_repo_layout.py`** in CI.
...
| File placement, packages, splits | **`docs/policy/repository-layout-policy.md`** |
...
1. **`docs/policy/METACOMPRESSOR_WORKING_CONTRACT.md`** (this file) — delta in “Last update” or short changelog below.
2. **`AGENTS.md`** — especially §5 freeze zones and §9 precedence if new ADRs landed.
3. **`docs/policy/repository-layout-policy.md`** — if folders or layout checks changed.
