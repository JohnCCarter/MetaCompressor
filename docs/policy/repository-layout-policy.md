...
5. **`docs/policy/METACOMPRESSOR_WORKING_CONTRACT.md`** — working anchor and “stay up to date” checklist (subordinate to `AGENTS.md`).
6. **This file** (`docs/policy/repository-layout-policy.md`) — repository layout and placement.
...
| `metacompressor/tests/` | Pytest suite only (domain subfolders under `metacompressor/tests/`; see “Layout rules for `metacompressor/tests/`”). |
| `benchmarks/` | Long-running validation, acceptance, and benchmark **drivers** under `benchmarks/<domain>/` (may import `metacompressor`). |
...
| Zone | Role |
|------|------|
| `docs/` | Prose: **`docs/policy/`** (governance + layout backlog + working contract), **`docs/planning/`** (roadmaps, phases, perf notes), **`docs/differential/`** (differential design/slices), **`docs/adr/`** (ADRs). See **`docs/README.md`**. |
...
- **Domain subfolders** (e.g. `core/`, `corpus/`, `differential/`, `validation/`, `stress/`) group related modules; keep **`conftest.py`** and shared helpers at the `tests/` root.
...
- **Subfolders by driver family:** `benchmarks/acceptance/` (acceptance + production validation), `benchmarks/differential/` (differential gates and simulations), `benchmarks/corpus/` (corpus-mode benches).
...
- **`docs/policy/`** — layout policy, structure backlog, working contract (`METACOMPRESSOR_WORKING_CONTRACT.md`).
- **`docs/planning/`** — roadmaps, phased rollout notes, performance summaries.
- **`docs/differential/`** — differential compression design, checklists, slice write-ups.
- **`docs/adr/`** — accepted architecture decisions (`0001-…`, etc.).
...
- Prefer the **domain subfolders** documented in **`results/README.md`** (`phases/`, `differential/`, `hardening/`, `product/`, `corpus/`) so the tree stays navigable; drivers should default to the matching folder.
...
1. **Drivers** and validation logic live in `benchmarks/<domain>/`; **outputs** go to `results/<domain>/` (or CLI-specified dirs).
...
3. Use **domain subfolders** (`core/`, `corpus/`, `differential/`, `validation/`, `stress/`) for navigation; keep shared fixtures and `conftest.py` at `metacompressor/tests/` root unless a subdirectory-local `conftest.py` is clearly justified.

## Layout rules for `docs/`, `.github/`, `.cursor/`

1. **`docs/`** — use **`docs/policy/`**, **`docs/planning/`**, **`docs/differential/`**, plus **`docs/adr/`** for ADRs; see **`docs/README.md`**.
2. **Keep policy distinct** from ADR content: layout guidance under `docs/policy/`; decisions that change structure in `docs/adr/`.
...
3. **Temporary** work → gitignored **`tmp/`** (`tmp/scripts/`, `tmp/logs/`, …); legacy `tmp_*.py` at repo root should be moved under **`tmp/scripts/`**. Use **`mc_test_output/`** for CLI dumps (gitignored).
...
- **Subfolder / subpackage** (e.g. `metacompressor/foo/bar.py`) — only when several modules form **one cohesive area** (shared imports, one clear façade, tests that naturally group) and **flat siblings** would be noisy or ambiguous. **Do not** create packages early just because there are “multiple files in the same category”—two or three well-named siblings at the package root are usually better than an empty-looking folder hierarchy. Rule of thumb: consider a subpackage when **~4+** closely related modules *or* cohesion/import pain appears—whichever comes first. **Today** CI allows **flat** `metacompressor/*.py` plus the intentional single-level subpackage **`metacompressor/differential/`** (machine-enforced in **`scripts/check_repo_layout.py`**). Any *additional* nested layout under `metacompressor/` requires updating that script, this document, and usually an ADR.
...
## Current repository audit (2026-05-08)

**Aligned with this policy**

- **`metacompressor/*.py`** library plus **`metacompressor/differential/*.py`** (one nested subpackage for the differential path); pytest under **`metacompressor/tests/`** with domain subfolders (`core/`, `corpus/`, `differential/`, `validation/`, `stress/`).
- **`benchmarks/<domain>/`** holds validation drivers; **`results/<domain>/`** holds report artifacts (see **`results/README.md`**).
- **`scripts/`** holds CI layout check; **`.github/workflows/`** runs layout check, **Ruff** (`ruff check`), **Black** (`black --check`), and **pytest**.
- **`docs/policy/`**, **`docs/planning/`**, **`docs/differential/`**, and **`docs/adr/`**; **`.cursor/`** for agent assets.
